...
| L2 评估 + 状态表 | ✅ |
| L3 改进 + 回归闸门 | ✅ |
| LLM 接入层(多供应商 / 重试 / 缓存 / 预算熔断 / 用量入表) | ✅ 34 条测试,用假 HTTP 钉死行为 |
| **对着真实模型端点跑一次** | ⚠️ 代码路径完整,**未在真实 workspace 上执行过** |
| 在真实 Databricks workspace 上跑通 | ⚠️ 同上 |

`243 passed`。测试可切换运行目标,`--rca-target=spark` 即在 Databricks 上复核同一批断言。
