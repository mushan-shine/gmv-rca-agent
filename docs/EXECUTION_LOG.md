...
| **`03` 的半自动改进可能泄露留出集答案**(写设计文档、逐模块核对代码时发现) | 注释写着留出集不参与,但给 few-shot 用的「参考 SQL 映射」包含了留出集;留出集的题答错时,它的标准 SQL 会被加进 prompt,下一次评估就是在抄答案 | 映射里排除留出集;测试把示例上限调大,确认留出集题仍然不会被加入。去掉修复时这条测试会失败 |

#### I4. 验证

- 本地测试 **352 passed**(新增 40 条:状态表迁移 1 条、分流 28 条(不需要数据库)、OAuth 连接 2 条、端到端 9 条,包括归因数字与直接写 SQL 算出的一致、
  查数经过自修复、失败和 👎 进入待复核队列、线上提示来自合并后的 `prompting.yaml`)
- 网页入口在本机(DuckDB + 样例数据)用浏览器实际点过:归因、过滤、明细表、👎 反馈、待复核队列
- `02` 的期望数字更新为 **`198 passed, 1 skipped`** 和 **`143 passed, 1 skipped`**
...
2. Pull 最新代码,**重新运行 `02`**(期望 `198 passed, 1 skipped` 和 `143 passed, 1 skipped`),再**重新运行 `04`**:
