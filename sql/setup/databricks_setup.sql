...
SELECT
  product_id,
  category,
  category || '-' || CAST(sub_idx AS STRING) AS sub_category,
  price_tier
FROM (
  SELECT
    product_id,
    (CASE WHEN (pmod(hash('cat|' || CAST(product_id AS STRING)), 1000000) / 1000000.0) < 0.240000 THEN 'Apparel' WHEN (pmod(hash('cat|' || CAST(product_id AS STRING)), 1000000) / 1000000.0) < 0.420000 THEN 'Electronics' WHEN (pmod(hash('cat|' || CAST(product_id AS STRING)), 1000000) / 1000000.0) < 0.580000 THEN 'Home' WHEN (pmod(hash('cat|' || CAST(product_id AS STRING)), 1000000) / 1000000.0) < 0.720000 THEN 'Beauty' WHEN (pmod(hash('cat|' || CAST(product_id AS STRING)), 1000000) / 1000000.0) < 0.850000 THEN 'Sports' WHEN (pmod(hash('cat|' || CAST(product_id AS STRING)), 1000000) / 1000000.0) < 0.940000 THEN 'Toys' ELSE 'Grocery' END) AS category,
    1 + CAST(FLOOR((pmod(hash('sub|' || CAST(product_id AS STRING)), 1000000) / 1000000.0) * 4) AS INT) AS sub_idx,
    (CASE WHEN (pmod(hash('tier|' || CAST(product_id AS STRING)), 1000000) / 1000000.0) < 0.400000 THEN 'BUDGET' WHEN (pmod(hash('tier|' || CAST(product_id AS STRING)), 1000000) / 1000000.0) < 0.820000 THEN 'MID' ELSE 'PREMIUM' END) AS price_tier
  FROM (SELECT CAST(n AS BIGINT) AS product_id FROM (SELECT explode(sequence(1, 240)) AS n) _s) p
) q;
...
SELECT
  ...
  market,
  date_add(DATE '2025-05-05', -CAST(FLOOR((pmod(hash('signup|' || CAST(user_id AS STRING)), 1000000) / 1000000.0) * 730) AS INT)) AS signup_dt,
  (CASE WHEN (pmod(hash('utier|' || CAST(user_id AS STRING)), 1000000) / 1000000.0) < 0.450000 THEN 'NEW' WHEN (pmod(hash('utier|' || CAST(user_id AS STRING)), 1000000) / 1000000.0) < 0.870000 THEN 'RETURNING' ELSE 'VIP' END) AS user_tier
FROM (
  SELECT user_id, (CASE WHEN user_id <= 27500 THEN 'US' WHEN user_id <= 40000 THEN 'UK' ELSE 'DE' END) AS market
  FROM (SELECT CAST(n AS BIGINT) AS user_id FROM (SELECT explode(sequence(1, 50000)) AS n) _s) u
) m;
...
WITH
  ...
  grid AS (
    SELECT d.dt, c.channel, v.device, m.market,
           CAST(GREATEST(1, ROUND(6000 * (CASE c.channel WHEN 'Paid Search' THEN 0.3 WHEN 'Organic' THEN 0.28 WHEN 'Email' THEN 0.1 WHEN 'Social' THEN 0.17 WHEN 'Direct' THEN 0.15 END) * (CASE v.device WHEN 'mobile' THEN 0.62 WHEN 'desktop' THEN 0.38 END) * (CASE m.market WHEN 'US' THEN 0.55 WHEN 'UK' THEN 0.25 WHEN 'DE' THEN 0.2 END) * (CASE (datediff(d.dt, DATE '2025-05-05') % 7) WHEN 0 THEN 1.0 WHEN 1 THEN 0.98 WHEN 2 THEN 0.97 WHEN 3 THEN 0.99 WHEN 4 THEN 1.04 WHEN 5 THEN 1.12 WHEN 6 THEN 1.08 END) * (1 + 0.0015 * datediff(d.dt, DATE '2025-05-05')) * (0.92 + 0.16 * (pmod(hash(concat_ws('|', CAST(d.dt AS STRING), c.channel, v.device, m.market, 'sessions')), 1000000) / 1000000.0)))) AS INT) AS n_sessions
    FROM dates d
  ...
  expanded AS (
    SELECT g.dt, g.channel, g.device, g.market,
           explode(sequence(1, g.n_sessions)) AS i
    FROM grid g
  ),
  sess AS (
    SELECT dt, channel, device, market, concat_ws('_', 's', replace(CAST(dt AS STRING), '-', ''), (CASE channel WHEN 'Paid Search' THEN 'PS' WHEN 'Organic' THEN 'OR' WHEN 'Email' THEN 'EM' WHEN 'Social' THEN 'SO' WHEN 'Direct' THEN 'DI' END) || (CASE device WHEN 'mobile' THEN 'M' WHEN 'desktop' THEN 'D' END), market, lpad(CAST(i AS STRING), 6, '0')) AS session_id
    FROM expanded
  )
SELECT
  session_id,
  (CASE market WHEN 'US' THEN 1 + CAST(FLOOR((pmod(hash('user|' || session_id), 1000000) / 1000000.0) * 27500) AS BIGINT) WHEN 'UK' THEN 27501 + CAST(FLOOR((pmod(hash('user|' || session_id), 1000000) / 1000000.0) * 12500) AS BIGINT) WHEN 'DE' THEN 40001 + CAST(FLOOR((pmod(hash('user|' || session_id), 1000000) / 1000000.0) * 10000) AS BIGINT) END) AS user_id,
  channel,
  device,
  (CASE market WHEN 'US' THEN 'NA' WHEN 'UK' THEN 'EMEA' WHEN 'DE' THEN 'EMEA' END) AS region,
  market,
  ...
  (((pmod(hash(session_id || '|conv'), 1000000) / 1000000.0)) < (0.045 * (CASE channel WHEN 'Paid Search' THEN 0.9 WHEN 'Organic' THEN 1.15 WHEN 'Email' THEN 1.4 WHEN 'Social' THEN 0.6 WHEN 'Direct' THEN 1.25 END) * (CASE device WHEN 'mobile' THEN 0.8 WHEN 'desktop' THEN 1.3 END) * (CASE market WHEN 'US' THEN 1.0 WHEN 'UK' THEN 0.95 WHEN 'DE' THEN 0.9 END) * (0.94 + 0.12 * (pmod(hash(concat_ws('|', CAST(dt AS STRING), channel, device, market, 'cvr')), 1000000) / 1000000.0))) AND ((pmod(hash(session_id || '|status'), 1000000) / 1000000.0)) < 0.92) AS converted,
  dt
FROM sess;
...
WITH
  ...
  campaigns AS (SELECT explode(sequence(1, 3)) AS camp_no)
SELECT
  concat_ws('-', 'cmp', (CASE channel WHEN 'Paid Search' THEN 'PS' WHEN 'Social' THEN 'SO' END),
            market, lpad(CAST(camp_no AS STRING), 2, '0')) AS campaign_id,
  channel,
  market,
  ...
  dt
FROM (
  SELECT d.dt, c.channel, m.market, k.camp_no, CAST(GREATEST(1, ROUND(6000 * (CASE c.channel WHEN 'Paid Search' THEN 0.3 WHEN 'Social' THEN 0.17 END) * (CASE m.market WHEN 'US' THEN 0.55 WHEN 'UK' THEN 0.25 WHEN 'DE' THEN 0.2 END) * (CASE (datediff(d.dt, DATE '2025-05-05') % 7) WHEN 0 THEN 1.0 WHEN 1 THEN 0.98 WHEN 2 THEN 0.97 WHEN 3 THEN 0.99 WHEN 4 THEN 1.04 WHEN 5 THEN 1.12 WHEN 6 THEN 1.08 END) / 3.0 * (0.85 + 0.30 * (pmod(hash(concat_ws('|', CAST(d.dt AS STRING), c.channel, m.market, CAST(k.camp_no AS STRING), 'spend')), 1000000) / 1000000.0)))) AS BIGINT) AS clicks
  FROM dates d
  CROSS JOIN channels c
  CROSS JOIN markets m
